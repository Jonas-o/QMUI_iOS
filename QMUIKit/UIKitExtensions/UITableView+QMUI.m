...
- (CGFloat)qmui_validContentWidth {
    ...
    BOOL isInsetGroupedStyle = NO;
    if (@available(iOS 13.0, *)) {
        isInsetGroupedStyle = self.style == UITableViewStyleInsetGrouped;
    } else {
        isInsetGroupedStyle = self.qmui_style == QMUITableViewStyleInsetGrouped;
    }
    CGFloat rightInset = MAX(self.safeAreaInsets.right + (isInsetGroupedStyle ? self.qmui_insetGroupedHorizontalInset : 0), CGRectGetWidth(indexFrame));
    CGFloat leftInset = self.safeAreaInsets.left + (isInsetGroupedStyle ? self.qmui_insetGroupedHorizontalInset : 0);
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UITableView class], NSSelectorFromString(@"_configureCellForDisplay:forIndexPath:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITableView *selfObject, UITableViewCell *cell, NSIndexPath *indexPath) {
                ...
                if (selfObject.isInsetGroupedStyle) {
        ...
        if (@available(iOS 13.0, *)) {
            ...
            OverrideImplementation([UITableViewCell class], NSSelectorFromString([NSString qmui_stringByConcat:@"_setContentClipCorners", @":", @"updateCorners", @":", nil]), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UITableViewCell *selfObject, CACornerMask firstArgv, BOOL secondArgv) {
                    ...
                    if (tableView.isInsetGroupedStyle) {
...
- (BOOL)isInsetGroupedStyle {
    if (@available(iOS 13.0, *)) {
        return self.style == UITableViewStyleInsetGrouped;
    }
    return self.qmui_style == QMUITableViewStyleInsetGrouped;
}
...
- (void)setQmui_insetGroupedCornerRadius:(CGFloat)qmui_insetGroupedCornerRadius {
    ...
    if (self.isInsetGroupedStyle && self.indexPathsForVisibleRows.count) {
        [self reloadData];
    }
}
